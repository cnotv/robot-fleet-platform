...
* `deploy/vm/`: Traefik proxy and a Compose override for one VM, deployed over SSH with `make vm-*`.
* `docs/`: VitePress documentation site, published to GitHub Pages.
* `Makefile`: `make up` and `make test` for Compose, `make vm-deploy` for a VM, `make k8s-local` for Kubernetes.
* `ingestion-service/cmd/simulator/hotel-fleet.json`: demo hotels and robots, used by the simulator and seeded by the API. Generated by `generate_fleet.py` in the same folder; edit the script, not the JSON.
